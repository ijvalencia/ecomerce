...
var iva;
var sesion = "";
var dato, txtcantidad, correosGmail, correo, id_gmailProducto;
var direcion = 0, numeroorden, subtotal;
var txtcantidad0 = 0;
var sub = 0;
var sub_iva = 0;
var Cadena1 = /[A-Za-z]+/;
var bandera31 = false, bandera222 = false;
var numerico1 = /[0-9]+/;
...
var correocompra = /[-A-Za-z0-9._]+[@][A-Za-z]+[.]{1}[A-Za-z]+/;
...
$(document).ready(function () {
    ...
    $.getJSON("../../bin/ingresar.php?categoria=envios", function (dato) {
        ...
        number = field[2];
        var str = number;
        id_gmailProducto = str.substr(6, 6);
        correosGmail = field[3];
        
        /*if(correosGmail===null){
            $("#txtclavescompra").attr("disabled", false); 
        }else{
             $("#txtclavescompra").attr("disabled", true);
             $("#txtemailcompra").val(id_gmailProducto);
        }*/
        //alert(correosGmail);
        $('.loader').fadeOut("slow");
    });
        ...
        if ((number === "") || (nombre === "invitado") || (number === undefined) || (number === "0")) {
            ...
            $.ajax({
                ...
                success: function (mnscorreo) {
                    ...
                    correo = String(mnscorreo[0]["correo"]);
                    $("#txtemailcompra").val(correo);

                    // jAlert("Acaba de inicar con su cuenta de la tienda");
                ...
                success: function (mnsdireccion) {
                    ...
                    if (mnsdireccion === "[]") {
                        ...
                        $('#btn-direccion').on("click", function () {
                            ...
                            //acomodarlo ojo de direciones                           
                        });
                        if (correosGmail !== "") {
                            $("#txtemailcompra").val(correosGmail);
                            $("#txtclavescompra").val(id_gmailProducto);
                              $.ajax({
                                      type: "POST",
                                      url: "../../bin/ingresar.php?categoria=registroGmail",                                 
                                      data:{"nombre": nombre,"apellido":txtapellido=0,"correos":correosGmail,"contrasena":id_gmailProducto},
                                success: function(gmailmsj){
                                    alert(gmailmsj);
                                    switch(gmailmsj) {
                                        case '0':
                                            jAlert("el correo ya estar registrado");
                                        break;
                                        case '1':
                                            jAlert("Se registro el con exito");
                                        break;
                                        default :
                                            jAlert("favor intentarlo de nuevo");
                                        break;    
                                    }
                                    }
                                });             
    ...
    $('#btn_confirmar_compra').on("click", function () {
        ...
        if ((sesion === "") || (sesion === "invitado") || (sesion === null)) {
            ...
            if (parseFloat($('#txt_total').text()) > parametros["compra_maxima"]) {
                ...
                // cuentacorreos = correosGmail;
                // jAlert("Tu Correo es GMAIL");
                var cuentacorreos = $("input:text[id='txtemailcompra']").val();
                var cuentaclave = $("input:password[id='txtclavescompra']").val();
                var checkrobot = $("#g-recaptcha-response").val();

                if ((cuentaclave === "") || (checkrobot === " ")) {
                    $('#txtclavescompra').css({"border": "2px solid red"});
                    bandera222 = false;

                } else {
                    $('#txtclavescompra').css({"border": "2px solid Gainsboro"});
                    ...
                    if (cuentaclave !== " ") {
                        ...
                        if (bandera222 === true) {
                            $.ajax({
                                ...
                                data: {"usuariocorreo": cuentacorreos, "usuarioclave": cuentaclave, "checkrobot": checkrobot},
                                success: function (mnscompara) {
                                    console.log(mnscompara);
                                    switch (mnscompara) {
                                        case 'c':
                        ...
                        success: function (mns) {
                            ...
                            $.ajax({
                                ...
                                success: function (mnss) {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        url: "../../bin/ingresar.php?categoria=idCorreo",
                                        data: {"idcorreosenviados": number},
                                        success: function (mnsdireccion) {
                                          switch(mnsdireccion){   
                                              case '0':
                                               jAlert("No pudo enviarse su correo su compra");
                                              break;
                                              case '1':
                                               jAlert("Compra enviada por correo");
                                              break;
                                              case '2':
                                               jAlert("Mensaje No Enviado");
                                              break;
                                              default:
                                               jAlert("ocurrio un error fabor de intentarlo");
                                              break;
                                          }                                         
                                        }
                                    });
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        url: "../../bin/ingresar.php?categoria=registrarconGmail",
                                        data: {"idcorreoGmail":number,"correoGmail":nombre,"idContraseñagmail":correosGmail},
                                        success: function (mnsdireccion) {
                                          switch(mnsdireccion){   
                                              case '0':
                                               jAlert("No pudo enviarse su correo su compra");
